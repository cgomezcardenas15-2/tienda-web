...
export default async function ProductosAdminPage() {
  ...
  const [{ data: productos, error }, { data: variantes }] = await Promise.all([
    supabaseAdmin.from("productos").select("id,nombre,sku,stock,activo,imagen_url").order("nombre"),
  ...
  return <main className="mx-auto max-w-7xl px-5 py-8">
    <div className="flex flex-wrap items-end justify-between gap-4">
      ...
      <Link href="/admin/pedidos" className="rounded-xl border border-zinc-700 px-4 py-2 text-sm font-bold hover:border-lime-400">Ver pedidos</Link>
    </div>
    {error ? <p className="mt-7 rounded-xl bg-red-950 p-5 text-red-300">No fue posible cargar los productos.</p> :
      <div className="mt-7 grid gap-4 md:grid-cols-2">{(productos ?? []).map((producto) =>
        <article key={producto.id} className="flex items-center gap-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
          ...
          <div className="min-w-0 flex-1"><h2 className="truncate font-black">{producto.nombre}</h2><p className="mt-1 text-xs text-zinc-500">SKU: {producto.sku || "Sin SKU"} · Stock base: {producto.stock}</p><p className="mt-2 text-sm font-bold text-lime-400">{conteo.get(producto.id) ?? 0} variantes</p></div>
