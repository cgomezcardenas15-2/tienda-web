...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  hoja.mergeCells("A4:E4"); hoja.getCell("A4").value = `${IDENTIDAD_COMERCIAL.propietario} · NIT ${IDENTIDAD_COMERCIAL.nitCompleto} · ${IDENTIDAD_COMERCIAL.responsabilidadIva}`; hoja.getCell("A4").font = { size: 10, color: { argb: "FF666666" } };
  hoja.mergeCells("A5:E5"); hoja.getCell("A5").value = `Matrícula ${IDENTIDAD_COMERCIAL.matriculaMercantil} · CIIU ${IDENTIDAD_COMERCIAL.actividadPrincipal} / ${IDENTIDAD_COMERCIAL.actividadSecundaria} · ${IDENTIDAD_COMERCIAL.direccionCompleta}`; hoja.getCell("A5").font = { size: 10, color: { argb: "FF666666" } };
  ...
  hoja.getCell(`A${inicioTotales + 6}`).value = `${IDENTIDAD_COMERCIAL.nombreComercial} - Remisión de venta`; hoja.getCell(`A${inicioTotales + 6}`).font = { italic: true, size: 9, color: { argb: "FF777777" } }; hoja.mergeCells(`A${inicioTotales + 6}:E${inicioTotales + 6}`);
