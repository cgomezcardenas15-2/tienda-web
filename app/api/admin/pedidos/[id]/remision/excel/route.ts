...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  hoja.mergeCells("A3:E3"); hoja.getCell("A3").value = `REMISIÓN REM-${pedido.numero_pedido}`; hoja.getCell("A3").font = { bold: true, size: 14, color: { argb: "FF579900" } };
  hoja.getCell("A5").value = "Cliente"; hoja.getCell("B5").value = textoRemision(pedido.facturacion_razon_social || pedido.facturacion_nombre || pedido.comprador_nombre);
  hoja.getCell("A6").value = "Documento"; hoja.getCell("B6").value = `${textoRemision(pedido.comprador_tipo_documento)} ${textoRemision(pedido.comprador_numero_documento)}`;
  hoja.getCell("A7").value = "Correo"; hoja.getCell("B7").value = textoRemision(pedido.comprador_correo);
  hoja.getCell("A8").value = "Entrega"; hoja.getCell("B8").value = `${textoRemision(pedido.entrega_direccion)}, ${textoRemision(pedido.entrega_ciudad)}, ${textoRemision(pedido.entrega_departamento)}`;
  hoja.mergeCells("B5:E5"); hoja.mergeCells("B6:E6"); hoja.mergeCells("B7:E7"); hoja.mergeCells("B8:E8");
  const cabecera = hoja.getRow(10); cabecera.values = ["Producto", "SKU", "Cantidad", "Valor unitario", "Total"];
  ...
  hoja.getCell(`A${inicioTotales + 6}`).value = "Remisión comercial - Documento no fiscal"; hoja.getCell(`A${inicioTotales + 6}`).font = { italic: true, size: 9, color: { argb: "FF777777" } }; hoja.mergeCells(`A${inicioTotales + 6}:E${inicioTotales + 6}`);
